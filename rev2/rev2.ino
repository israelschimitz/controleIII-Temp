...
///-------------apagar--------------
    ...
    if(ligado > 0){
        ...
        if (!(temperatura > 900) && (temperatura < 1)) {
        ...
        if( potencia >= 0){
            ...
            potencia1 = map(potencia,0,800,0,100); /// Ajuste entre a potência do atuador
        ...
        unsigned long agora2 = agora + 100;
        while(agora2 >= agora1){
            agora1 = millis();
        ...
        int potenciaLCD = potencia1*200;
        lcd.clear();
        lcd.setCursor(0,0);         //Posição do cursor
        lcd.print(potenciaLCD);
...
float fcfuzpdp(signed int e, signed int v){
    ...
    signed int caZE = 0;
    signed int caPP = 30;
    signed int caPM = 60; //valor mudado de 60 para 80
    signed int caPG = 100;
